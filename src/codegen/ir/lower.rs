    ...
    fn lower_op(&mut self, op: &IROp, out: &mut Vec<String>) {
        match op {
            ...
            IROp::IfElse {
                ...
                // Miden: inline if.true / {then} / else / {else} / end
            ...
            IROp::Loop { label: _, body } => {
                // Miden loops: check if counter is 0, if so drop and return;
                // otherwise decrement, run body, recurse.
                self.emit(out, "dup.0");
...
mod tests {
    ...
    fn test_miden_fn_structure() {
        ...
        // Return is a no-op in Miden (uses end)
        assert_eq!(out[6], "end");
        assert_eq!(out[7], "");
    }
    ...
    fn test_miden_loop() {
        ...
        let out = lowering.lower(&ops);
        let joined = out.join("\n");
        // Should have inline loop with exec.self, not a separate subroutine
        assert!(joined.contains("dup.0"));
        assert!(joined.contains("push.0"));
    ...
    fn test_miden_nested_indent() {
        ...
        let lowering = MidenLowering::new();
        let out = lowering.lower(&ops);
        // Check that nested if gets double-indented
