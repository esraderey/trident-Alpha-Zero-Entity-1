...
enum RegistryAction {
    ...
    /// Initialize an on-chain Merkle registry tree
    OnChainInit,
    /// Register a definition in the on-chain Merkle tree
    OnChainRegister {
        /// Name or content hash of the UCM definition to register
        name: String,
    },
    /// Verify that a definition is in the on-chain Merkle tree
    OnChainVerify {
        /// Name or content hash to verify membership of
        name: String,
    },
    /// Update the verification certificate for a definition in the on-chain tree
    OnChainCertify {
        /// Name or content hash to certify
        name: String,
        /// Input .tri file to run verification on
        #[arg(short, long)]
        input: PathBuf,
    },
    /// Show the on-chain Merkle tree state (root, entries, proofs)
    OnChainStatus,
}
...
fn cmd_registry(action: RegistryAction) {
    match action {
        ...
        RegistryAction::OnChainInit => cmd_onchain_init(),
        RegistryAction::OnChainRegister { name } => cmd_onchain_register(name),
        RegistryAction::OnChainVerify { name } => cmd_onchain_verify(name),
        RegistryAction::OnChainCertify { name, input } => cmd_onchain_certify(name, input),
        RegistryAction::OnChainStatus => cmd_onchain_status(),
    }
}
...
// --- trident registry on-chain ---

fn onchain_state_dir() -> PathBuf {
    std::env::var("HOME")
        .map(PathBuf::from)
...
fn load_onchain_registry() -> trident::onchain::MerkleRegistry {
    let dir = onchain_state_dir();
    let state_file = dir.join("tree.json");
    if state_file.exists() {
        // Load from persisted state
        let _data = std::fs::read_to_string(&state_file).unwrap_or_default();
        // For now, return empty — full deserialization is a future enhancement.
        // The tree is rebuilt from the UCM on each session.
        trident::onchain::MerkleRegistry::new()
    } else {
        trident::onchain::MerkleRegistry::new()
    }
}

fn save_onchain_state(registry: &trident::onchain::MerkleRegistry) {
    let dir = onchain_state_dir();
...
fn cmd_onchain_init() {
    ...
    let registry = trident::onchain::MerkleRegistry::new();
    save_onchain_state(&registry);
    println!("On-chain registry initialized");
    println!("  Root:     {}", registry.root().to_hex());
    println!("  Depth:    {}", trident::onchain::TREE_DEPTH);
    println!("  Capacity: {} entries", trident::onchain::MAX_ENTRIES);
    println!("  State:    {}", dir.join("tree.json").display());
}

fn cmd_onchain_register(name: String) {
    // Load UCM codebase
    let codebase = match trident::ucm::Codebase::open() {
        Ok(cb) => cb,
        Err(e) => {
            eprintln!("error: could not open UCM codebase: {}", e);
            process::exit(1);
        }
    };
    let def = match codebase.lookup(&name) {
        Some(d) => d,
        None => {
            eprintln!("error: '{}' not found in UCM codebase", name);
            eprintln!("hint: run `trident ucm add <file>` first");
            process::exit(1);
        }
    };

    // Compute content hash for this definition
    let hash = match codebase.hash_for_name(&name) {
        Some(h) => h,
        None => {
            eprintln!("error: could not resolve hash for '{}'", name);
            process::exit(1);
        }
    };
    ...
    let type_sig = format!(
        ...
        def.params
            ...
            .collect::<Vec<_>>()
            .join(", "),
        def.return_ty.as_deref().unwrap_or("()")
    );
    let type_sig_hash = {
        let h = blake3::hash(type_sig.as_bytes());
    ...
    let deps_hash = {
        ...
        for dep in &def.dependencies {
            dep_bytes.extend_from_slice(&dep.0);
        }
        if dep_bytes.is_empty() {
            0u64
        } else {
            let h = blake3::hash(&dep_bytes);
            let bytes = h.as_bytes();
            u64::from_le_bytes([
                bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], 0, 0,
            ])
    ...
    let entry = trident::onchain::RegistryEntry {
        content_hash: *hash,
        type_sig_hash: trident::onchain::FieldElement::new(type_sig_hash),
        deps_hash: trident::onchain::FieldElement::new(deps_hash),
        certificate: None,
        metadata: trident::onchain::EntryMetadata {
            publisher_auth: trident::onchain::FieldElement::zero(),
            timestamp,
            tag_hash: trident::onchain::FieldElement::zero(),
            name_hash: trident::onchain::FieldElement::new({
    ...
    let mut onchain = load_onchain_registry();
    match trident::onchain::generate_register_proof(&mut onchain, entry, Some(&name)) {
        Ok(proof) => {
            save_onchain_state(&onchain);
            println!("Registered '{}' in on-chain Merkle tree", name);
            println!("  Hash:       {}", hash.to_hex());
            println!("  Leaf index: {}", proof.leaf_index);
            println!("  Old root:   {}", proof.old_root.to_hex());
            println!("  New root:   {}", proof.new_root.to_hex());
            println!("  Proof JSON: {}", proof.to_json());
        }
        Err(e) => {
            eprintln!("error: {}", e);
            process::exit(1);
...
fn cmd_onchain_verify(name: String) {
    let codebase = match trident::ucm::Codebase::open() {
        Ok(cb) => cb,
        Err(e) => {
            eprintln!("error: could not open UCM codebase: {}", e);
    ...
    let onchain = load_onchain_registry();
    match trident::onchain::generate_membership_proof(&onchain, &hash) {
        Ok(proof) => {
            println!("Definition '{}' is registered in on-chain tree", name);
            println!("  Root:       {}", proof.root.to_hex());
            println!("  Leaf index: {}", proof.leaf_index);
            println!("  Cert hash:  {}", proof.cert_hash);
            let verified = proof.cert_hash.0 != 0;
...
fn cmd_onchain_certify(name: String, input: PathBuf) {
    // Run verification on the source file
    ...
    let verdict = match report.verdict {
        trident::solve::Verdict::Safe => trident::onchain::OnChainVerdict::Safe,
        trident::solve::Verdict::StaticViolation => {
            trident::onchain::OnChainVerdict::StaticViolation
        }
        trident::solve::Verdict::RandomViolation => {
            trident::onchain::OnChainVerdict::RandomViolation
        }
        trident::solve::Verdict::BmcViolation => trident::onchain::OnChainVerdict::BmcViolation,
    };
    ...
    let cert = trident::onchain::VerificationCertificate {
        verdict,
        constraints_checked: report.random_result.constraints_checked as u64,
        rounds: report.random_result.rounds as u64,
        timestamp,
        verifier_auth: trident::onchain::FieldElement::zero(),
    };

    let mut onchain = load_onchain_registry();
    match trident::onchain::generate_update_cert_proof(&mut onchain, &hash, cert) {
        Ok(proof) => {
            save_onchain_state(&onchain);
            println!("Certificate updated for '{}' in on-chain tree", name);
...
fn cmd_onchain_status() {
    let onchain = load_onchain_registry();
    if onchain.is_empty() {
        println!("On-chain registry: empty");
        println!("  Root:     {}", onchain.root().to_hex());
        println!("  Capacity: {}", trident::onchain::MAX_ENTRIES);
        println!("\nhint: run `trident registry on-chain-init` to initialize");
        return;
    }

    println!("On-chain registry:");
    println!("  Root:     {}", onchain.root().to_hex());
    println!(
        "  Entries:  {}/{}",
        onchain.len(),
        trident::onchain::MAX_ENTRIES
    );
    println!();

    for (idx, entry) in onchain.entries() {
        let verified = entry.certificate.is_some();
        let hash_short = &entry.content_hash.to_hex()[..16];
        let verdict = entry
            .certificate
            .as_ref()
            .map(|c| format!("{}", c.verdict))
            .unwrap_or_else(|| "unverified".to_string());
        println!("  [{}] {}..  {}", idx, hash_short, verdict);
        if verified {
            let c = entry.certificate.as_ref().unwrap();
            println!(
                "       constraints={} rounds={} ts={}",
                c.constraints_checked, c.rounds, c.timestamp
            );
        }
    }
}
