...
//
// Flat input -> hidden -> decoder. ~1KB private memory per thread.
...
// Private memory: ~1KB total.
var<private> latent: array<f32, 32>;     // DIM
var<private> dec_h: array<f32, 32>;      // FFN
var<private> dec_out: array<f32, 64>;    // VOCAB
var<private> prev_out: array<f32, 64>;   // VOCAB
...
fn neural_forward(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var d = 0u; d < DIM; d++) {
        ...
        for (var n = 0u; n < seq_len; n++) {
            for (var ww = 0u; ww < WORDS_PER_NODE; ww++) {
                ...
                acc += block_nodes[b_base + flat_idx] * w(ind, IP_OFF + flat_idx * DIM + d);
            }
        }
    ...
    for (var d = 0u; d < DIM; d++) {
        ...
        for (var j = 0u; j < DIM; j++) {
            acc += latent[j] * w(ind, HW_OFF + j * DIM + d);
        }
        hidden[d] = max(acc, 0.0);
    }
    // Copy hidden to latent for decoder input
    for (var d = 0u; d < DIM; d++) { latent[d] = hidden[d]; }

    // 3. Autoregressive decoder
    for (var d = 0u; d < VOCAB; d++) { prev_out[d] = 0.0; }

    for (var step = 0u; step < MAX_OUTPUT; step++) {
        // Decoder hidden: [DIM + VOCAB] -> [FFN] + bias + ReLU
        for (var fh = 0u; fh < FFN; fh++) {
            var acc = w(ind, DHB_OFF + fh);
            for (var d = 0u; d < DIM; d++) {
                acc += latent[d] * w(ind, DH_OFF + d * FFN + fh);
            }
            for (var d = 0u; d < VOCAB; d++) {
                acc += prev_out[d] * w(ind, DH_OFF + (DIM + d) * FFN + fh);
            }
        ...
        for (var d = 0u; d < VOCAB; d++) {
            ...
            for (var fh = 0u; fh < FFN; fh++) {
                acc += dec_h[fh] * w(ind, DO_OFF + fh * VOCAB + d);
            }
            dec_out[d] = acc;
        }
        ...
        for (var d = 0u; d < VOCAB; d++) { prev_out[d] = dec_out[d]; }
    }
}
