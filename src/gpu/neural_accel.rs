//! GPU-accelerated neural forward pass (float32).
//!
...
impl NeuralAccelerator {
    pub fn try_create(max_blocks: u32, max_individuals: u32) -> Option<Self> {
        ...
        let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(shaders::NEURAL_FORWARD.into()),
        });
        ...
        let params_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("neural_params"),
            contents: bytemuck::bytes_of(&params),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });

        let weight_buf_size = max_individuals as u64 * WEIGHT_COUNT as u64 * 4;
        let weight_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("neural_weights"),
        ...
        let output_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("neural_outputs"),
            size: output_buf_size,
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
            mapped_at_creation: false,
        });
        let staging_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("neural_staging"),
            size: output_buf_size,
    ...
    pub fn batch_forward(&self, weight_vecs: &[Vec<Fixed>]) -> Vec<Vec<Vec<u32>>> {
        ...
        let mut weight_data: Vec<f32> =
            Vec::with_capacity(num_ind as usize * WEIGHT_COUNT as usize);
